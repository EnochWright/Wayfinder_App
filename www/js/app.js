...
function initGameModules() {
    // Lazy initialization - games are initialized when their page is navigated to
    console.log('[App] Game modules will be initialized on demand');
}

// Initialize a specific game module by page name
function initGameModule(pageName) {
    const gameMap = {
        'alchemists': 'AlchemistsGame',
        'catan': 'CatanGame',
        'container': 'ContainerGame',
        'newbedford': 'NewBedfordGame',
        'planetx': 'PlanetXGame',
        'railbaron': 'RailBaronGame',
        'rising5': 'Rising5Game',
        'campingseason': 'CampingSeasonGame'
    };

    const gameKey = gameMap[pageName];
    if (gameKey && window[gameKey]) {
        console.log(`[App] Initializing ${gameKey} for page: ${pageName}`);
        window[gameKey].init();
    }
}
...
async function switchPage(pageName) {
    ...
    if (targetPage) {
        ...
        // Initialize game module on demand
        initGameModule(pageName);
